...
e = 5
R = weighted1n2Veronese(2,e,ZZ/101)
use T
M = makeGuessXYmatrix(e,S);
-*ideal R == minors(2,*-turnToZiMatrixFromXY(M,S,T,mons)--)
...
L
use T
leadMons = apply(L, l -> leadMonomial l)
printXs = m -> (
    for z1 in gens T do (
        if m%z1 == 0 then return {mons_(-1+position(gens T, z -> z == z1)), mons_(-1+position(gens T, z -> z == m//z1))}
    );
)
netList apply(leadMons, m -> printXs m)
L
p = position(L, l -> (l_0 // y == l_1 // y))
